(function(global,factory){
  ...
  MapLineAir.prototype = {
    init : function(data){
        ...
        //设置起始点
        var startPoint = this._startPoint(data.data.vertexs[0].position);
    ...
    _addPlane:function(dataArr){
      var overlays = this.map.getOverlays();
      for(var n=0,leng=overlays.length; n<leng; n++){
        if(overlays[n].id=='plain'){
      ...
      for(var m=0,length=dataArr.length; m<length; m++){
        ...
        var marker = new BMap.Marker(point,{icon:new BMap.Icon("./img/plane.png",new BMap.Size(59,60),{
          imageOffset: new BMap.Size(0,0)
        })});
        if(Math.abs(xDeviation) == xDeviation && Math.abs(yDeviation) == -yDeviation){
